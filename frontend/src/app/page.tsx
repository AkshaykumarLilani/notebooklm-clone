...
import { UploadContextProvider } from "@/lib/context/UploadContext"
import axios from "axios"
import { Loader } from "lucide-react"
import { useEffect, useLayoutEffect, useState } from "react"

export default function Home() {
    const [isBaseUrlSet, setIsBaseUrlSet] = useState<boolean | null>(null)
    ...
    if (isBackendUp === false) {
        return (
            <div className="flex flex-col min-h-screen min-w-screen items-center justify-center text-center p-4">
                ...
                <p className="text-muted-foreground">{`The application could not connect to the backend after several attempts.`}</p>
                <p className="text-muted-foreground">{`You can try again by reloading the page or check back in later`}</p>
            </div>
        )
    }
    ...
    return (
        <div className="flex flex-col min-h-screen min-w-screen items-center justify-center text-center p-4">
            ...
            <p className="text-muted-foreground">{`The backend is hosted on a free service and may take up to 50 seconds to start up if it's been inactive.`}</p>
            <p className="text-muted-foreground mt-2">{`Please be patient.`}</p>
        </div>
    )
}
