...
const UploadModal = () => {
    ...
    const { handleUploadPdf, uploadError, isUploadInProgress, uploadProgress, uploadData, isUploadSheetOpen, setIsUploadSheetOpen, userUploadedPdf } = useUploadContext() as UploadContextType
        ...
        <Sheet open={isUploadSheetOpen} onOpenChange={setIsUploadSheetOpen}>
            ...
            >
                <SheetHeader className="min-w-[90vw] lg:min-w-[50vw] px-0">
                    ...
                    <SheetDescription>Drag and drop your PDF file here or click to browse.</SheetDescription>
                </SheetHeader>
                {uploadError && <p className="text-destructive mt-2 text-sm max-w-[50vw]">{uploadError?.error || uploadError.message}</p>}
                <div className={`relative border-2 border-dashed rounded-lg p-10 text-center min-w-[90vw] lg:min-w-[50vw] px-4 ${isDragging ? "border-primary" : "border-foreground"}`}>
                    {!isUploadInProgress ? (
                        <>
                            <UploadCloud className={`mx-auto h-12 w-12 text-foreground ${uploadProgress === 100 ? "" : ""}`} />
                            <p className="mt-2 text-sm text-foreground">{isDragging ? "Drop the file here" : "Drag and drop or"}</p>
                            <input placeholder="File" type="file" className="absolute top-0 left-0 h-full w-full opacity-0 cursor-pointer" onChange={handleFileChange} accept=".pdf" />
