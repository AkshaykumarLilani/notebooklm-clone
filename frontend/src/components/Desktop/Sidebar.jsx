...
export default function Sidebar() {
    const { pdfList, activePdfId, fetchPdfList, selectPdf } = useChatContext();
            ...
            {pdfList.length === 0 ? (
                ...
                <ul className="space-y-2">
                    {pdfList.map((pdf) => (
                        <li
                            key={pdf.pdf_id}
                            className={`cursor-pointer px-3 py-2 rounded-lg transition ${activePdfId === pdf.pdf_id
                                    ? 'bg-card text-primary font-bold'
                                    : 'hover:bg-muted'
                                }`}
                            onClick={() => selectPdf(pdf.pdf_id)}
                            title={pdf.filename}
                        >
